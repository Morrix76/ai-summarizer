 ...
 * @returns {Promise<Object>} - Object containing summary and truncation info
 */
export async function generateSummary(text, template = 'brief') {
  try {
    ...
    // Limit text length to avoid token limits with llama-3.1-8b-instant
    // Safe limit: 4000 characters (~1000 tokens) to prevent context overflow
    const maxChars = 4000;
    ...
    return {
      ...
      truncatedLength: wasTruncated ? maxChars : originalLength
    };
