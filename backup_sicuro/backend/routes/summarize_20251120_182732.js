...
router.post('/summarize-file', upload.single('file'), async (req, res) => {
  try {
    ...
    // Generate summary using Groq
    const result = await generateSummary(extractedText, template);
    ...
    res.json({
      ...
      warning: result.truncated 
        ? `⚠️ Il testo estratto dal file (${result.originalLength} caratteri) è stato troncato a ${result.truncatedLength} caratteri per rispettare i limiti del modello.`
        : null
    });
